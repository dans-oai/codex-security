...
test('finding titles and messages cannot become Markdown links, images or raw HTML',()=>{
  ...
  const summary = resultSummary(result,parseInputs(()=>'', '/checkout'),{repository:'/checkout',scannedSha:'a'.repeat(40),analysisRef:'refs/heads/main',publishable:true,emptyDiff:false,policyFiles:[]});
